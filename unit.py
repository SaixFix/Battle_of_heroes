...
class BaseUnit(ABC):
    ...
    def _count_damage(self, target: BaseUnit) -> float:
        """логику расчета урона игрока = урон_от_оружия * модификатор_атаки_класса
        логику расчета брони цели =  надетая_броня * модификатор_брони_класса
        ...
        required_stamina = target.armor.stamina_per_turn * target.unit_class.stamina

        if required_stamina < target.stamina:
            damage -= target.armor.defence
            target.stamina -= required_stamina
...
class EnemyUnit(BaseUnit):
    ...
    def hit(self, target: BaseUnit) -> str:
        ...
        """
        if not self._is_skill_used and randint(0, 100) < 10:
